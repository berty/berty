...
final class PeerDevice {
    ...
    void setLibp2pService(final BluetoothGattService service) {
    ...
    void disconnectFromDevice(final String cause) {
    ...
    private void setGatt() {
        ...
        if (dGatt == null) {
            dGatt = dDevice.connectGatt(BleDriver.getContext(), false, BleDriver.getGattCallback());
        }
    }

    private boolean connectGatt(final String caller) throws InterruptedException {
    ...
    private int getGattState(final boolean client) {
        final BluetoothManager manager = (BluetoothManager)BleDriver.getContext().getSystemService(Context.BLUETOOTH_SERVICE);
        if (manager == null) {
    ...
    private boolean libp2pHandshake(final String caller) throws Exception {
    ...
    boolean writeToRemoteWriterCharacteristic(final byte[] payload) throws InterruptedException {
