						...
						return (
							...
							>
								<View style={{ alignItems: 'center', flexDirection: 'row' }}>
									<Image style={{ height: 50, width: 50 }} source={{ uri: game.icon }} />
									<Text style={{ marginLeft: 20, color: 'black' }}>{game.name}</Text>
								</View>
...
const App = () => {
	...
	useEffect(() => {
		const f = async () => {
			...
			const directories =
				Platform.OS === 'ios' ? await RNFS.readDir(pkgDir) : await RNFS.readDirAssets('')
			...
			for (const dir of directories) {
				...
				if (
					...
					const zipContents =
						Platform.OS === 'ios'
							? await RNFS.readFile(pkgDir + `/${dir.name}` + `/${gzFile}`, 'base64')
							: await RNFS.readFileAssets(`${dir.name}/game.html`, 'utf8')

					// read manifest file
					const manifestContent = JSON.parse(
						Platform.OS === 'ios'
							? await RNFS.readFile(pkgDir + `/${dir.name}` + `/${manifestFile}`, 'utf8')
							: await RNFS.readFileAssets(`${dir.name}/${manifestFile}`, 'utf8'),
					)

					// unzip for iOS
					const unzipContents = Platform.OS === 'ios' ? Gzip.unzip(zipContents) : zipContents
			...
			{isGame && games.length ? (
				...
				<SafeAreaView style={{ ...backgroundStyle, flex: 1 }}>
					...
					<View style={{ margin: 30, ...backgroundStyle }}>
						<Text style={{ ...colorStyle, textAlign: 'center', fontWeight: '800', fontSize: 30 }}>
							BERTY MINI GAME
						</Text>
					</View>
