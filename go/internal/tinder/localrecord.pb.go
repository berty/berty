...
type LocalRecord struct {
	Records              map[string]int64 `protobuf:"bytes,1,rep,name=records,proto3" json:"records,omitempty" protobuf_key:"bytes,1,opt,name=key,proto3" protobuf_val:"varint,2,opt,name=value,proto3"`
...
func (m *LocalRecord) GetRecords() map[string]int64 {
	if m != nil {
		return m.Records
	}
	return nil
...
func init() {
	...
	proto.RegisterMapType((map[string]int64)(nil), "tinder.LocalRecord.RecordsEntry")
}
...
var fileDescriptor_74ce3f655004e39b = []byte{
	...
	,
}
...
func (m *LocalRecord) MarshalToSizedBuffer(dAtA []byte) (int, error) {
	...
	if len(m.Records) > 0 {
		for k := range m.Records {
			v := m.Records[k]
			baseI := i
			i = encodeVarintLocalrecord(dAtA, i, uint64(v))
			i--
			dAtA[i] = 0x10
			i -= len(k)
			copy(dAtA[i:], k)
			i = encodeVarintLocalrecord(dAtA, i, uint64(len(k)))
			i--
			dAtA[i] = 0xa
			i = encodeVarintLocalrecord(dAtA, i, uint64(baseI-i))
			i--
			dAtA[i] = 0xa
		}
	}
...
func (m *LocalRecord) Size() (n int) {
	...
	if len(m.Records) > 0 {
		for k, v := range m.Records {
			_ = k
			_ = v
			mapEntrySize := 1 + len(k) + sovLocalrecord(uint64(len(k))) + 1 + sovLocalrecord(uint64(v))
			n += mapEntrySize + 1 + sovLocalrecord(uint64(mapEntrySize))
		}
	}
	if m.XXX_unrecognized != nil {
