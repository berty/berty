...
func createNewIdentity(t *testing.T, ctx context.Context) (iface.Crypto, p2pCrypto.PrivKey) {
	...
	db, err = datastore.InitMigrate(db)
	assert.Nil(t, err)
	...
	c, privateKey, err := cryptoModule.InitNewIdentity(ctx, ds)
	assert.Nil(t, err)
...
func createTwoDevices(t *testing.T, ctx context.Context) (iface.HandshakeSession, iface.Crypto, iface.HandshakeSession, iface.Crypto) {
	...
	hss1, err := hs.NewRequest(pk1, c1.GetSigChain(), accountPublicKey)
	assert.Nil(t, err)
	...
	hss2, err := hs.NewResponse(pk2, c2.GetSigChain(), signBytes, box)
	assert.Nil(t, err)
...
func TestModule_NewRequest(t *testing.T) {
	...
	hss, err := hs.NewRequest(pk1, c1.GetSigChain(), accountPubKey)
	assert.Nil(t, err)
	assert.NotNil(t, hss)
}
...
func TestModule_NewResponse(t *testing.T) {
	...
	hss1, err := hs.NewRequest(pk1, c1.GetSigChain(), accountPubKey)
	assert.Nil(t, err)
	assert.NotNil(t, hss1)
...
func TestHandshakeSession_GetPublicKeys(t *testing.T) {
	...
	assert.Equal(t, int(sign.Type()), handshake.SupportedKeyType)
	assert.Len(t, box, 32)
}
