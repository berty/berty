...
func (d *pushService) Send(_ context.Context, request *protocoltypes.PushServiceSend_Request) (*protocoltypes.PushServiceSend_Reply, error) {
...
func (d *pushService) sendSingle(rawPayload []byte, receiver *protocoltypes.PushServiceOpaqueReceiver) error {
	...
	if err := d.dispatchers[pushDispatcherKey(pushReceiver.TokenType, pushReceiver.BundleID)].Dispatch(payloadBytes, pushReceiver); err != nil {
...
func decryptPushDataFromServer(data []byte, serverPK, ownSK *[cryptoutil.KeySize]byte) ([]byte, error) {
	if serverPK == nil {
	...
	if !ok {
		return nil, errcode.ErrPushUnableToDecrypt
	}
