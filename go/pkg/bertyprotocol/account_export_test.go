...
func TestFlappyRestoreAccount(t *testing.T) {
...

	{
		...
		op, err := serviceA.getAccountGroup().messageStore.AddMessage(ctx, testPayload1)
		require.NoError(t, err)

		expectedMessages[op.GetEntry().GetHash()] = testPayload1

		op, err = serviceA.getAccountGroup().messageStore.AddMessage(ctx, testPayload2)
		require.NoError(t, err)
...

	{
		...
		entries := nodeB.Service.(*service).getAccountGroup().messageStore.OpLog().GetEntries()
