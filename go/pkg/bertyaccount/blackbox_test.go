...
func TestFlow(t *testing.T) {
	...
	// try closing the account again, even if no account should be loaded anymore
	{
		...
		require.Equal(t, lastProgress.State, "done")
		require.True(t, lastProgress.Completed > 1)
		require.Equal(t, int(lastProgress.Completed), 15) // this test can be disabled if it breaks, the test just above can be considered as enough
		require.Equal(t, lastProgress.Completed, lastProgress.Total)
		require.Equal(t, lastProgress.Progress, float32(1))
...
func TestImportExportFlow(t *testing.T) {
	...
	{
		stream, err := cl.ImportAccountWithProgress(ctx, &bertyaccount.ImportAccountWithProgress_Request{
			AccountID:   "account 2",
		...
		steps := 0
		var lastProgress *protocoltypes.Progress
		var lastMsg *bertyaccount.ImportAccountWithProgress_Reply
		for {
			msg, err := stream.Recv()
			if err == io.EOF {
				break
