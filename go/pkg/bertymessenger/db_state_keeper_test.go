...
func Test_keepAccountStringField(t *testing.T) {
	db, dispose := getInMemoryTestDB(t, getInMemoryTestDBOptsNoInit)
	defer dispose()

	log := zap.NewNop()

	require.Equal(t, "", keepAccountStringField(db.db, "display_name", nil))
	...
	require.Equal(t, "", keepAccountStringField(db.db, "display_name", log))
	...
	require.Equal(t, "display_name_1", keepAccountStringField(db.db, "display_name", log))
...
func Test_keepAccountBoolField(t *testing.T) {
	db, dispose := getInMemoryTestDB(t, getInMemoryTestDBOptsNoInit)
	defer dispose()

	log := zap.NewNop()

	require.Equal(t, true, keepAccountBoolField(db.db, "replicate_new_groups_automatically", true, nil))

	// table schema on 2020_10_13
	require.NoError(t, db.db.Exec("CREATE TABLE accounts (public_key text, display_name text, link text, replicate_new_groups_automatically numeric DEFAULT true,PRIMARY KEY (public_key))").Error)

	require.Equal(t, true, keepAccountBoolField(db.db, "replicate_new_groups_automatically", true, log))
	...
	require.Equal(t, false, keepAccountBoolField(db.db, "replicate_new_groups_automatically", true, log))
