...
func Test_dbWrapper_getDBInfo(t *testing.T) {
	...
	require.Equal(t, int64(9), info.MetadataEvents)
	require.Equal(t, int64(10), info.Medias)
...
func Test_dbWrapper_updateConversationReadState(t *testing.T) {
	...
	require.NoError(t, db.db.Create(&messengertypes.Conversation{PublicKey: "convo_a", UnreadCount: 0, LastUpdate: timestampMs(time.Unix(10000, 0))}).Error)
	require.NoError(t, db.db.Create(&messengertypes.Conversation{PublicKey: "convo_b", UnreadCount: 1000, LastUpdate: timestampMs(time.Unix(20000, 0))}).Error)
	require.NoError(t, db.db.Create(&messengertypes.Conversation{PublicKey: "convo_c", UnreadCount: 2000, LastUpdate: timestampMs(time.Unix(30000, 0))}).Error)
...
func Test_dbWrapper_addServiceToken(t *testing.T) {
	...
	err = db.addServiceToken("", &protocoltypes.ServiceToken{
		Token:             "tok2",
		AuthenticationURL: "https://url2/",
		SupportedServices: []*protocoltypes.ServiceTokenSupportedService{},
	})
