...
test('Chat.ManageDeepLink renders ManageGroupInvitation when link kind is GroupV1Kind', async () => {
	...
	const { getByLabelText } = renderScreen('Chat.ManageDeepLink', ManageDeepLink, {
		type: 'link',
		value:
	...
	await waitFor(() => {
		expect(getByLabelText('ManageDeepLink-ManageGroupInvitation')).toBeDefined()
	})
})
...
test('Chat.ManageDeepLink renders AddThisContact when link kind is ContactInviteV1Kind', async () => {
	...
	await waitFor(() => {
		expect(getByLabelText('ManageDeepLink-AddThisContact')).toBeDefined()
	})
})
