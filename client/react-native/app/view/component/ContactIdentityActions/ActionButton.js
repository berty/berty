...
const ActionButton = ({ onPress, color, title, icon }) => (
  <View style={{ flex: 1 }}>
    <TouchableOpacity onPress={onPress || (() => {})}>
      ...
      />
      <Text center color={colors.white} tiny opacity={onPress ? 1 : 0.3}>
        {title}
      </Text>
    </TouchableOpacity>
  </View>
...
export const ActionButtonLarge = ({ onPress, color, title, icon }) => (
  <Text
    icon={icon}
    background={color}
    color={color === colors.white ? colors.subtleGrey : colors.white}
