...
class QRReader extends PureComponent {
  ...
  setStatus = permStatus => {
    const onPress =
      (Platform.OS === 'ios' && permStatus === permissionsStatus.denied) ||
      (Platform.OS === 'android' && permStatus === permissionsStatus.restricted)
    ...
    return permStatus === permissionsStatus.authorized ? (
      ...
      <View style={{ flexDirection: 'column' }}>
        ...
        <View style={{ flexGrow: 1 }}>
          <Button
            onPress={onPress}
            title={t('contacts.add.qrcode-camera-ask-button')}
          />
        </View>
      </View>
    )
