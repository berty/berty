...
function check(database, cb) {
  ...
  fs.readFile(path.join(dataPath, database.type + ".checksum"), function(
    ...
    function onResponse(response) {
      ...
      if (status !== 200) {
        console.log(
          "ERROR".red + ": HTTP Request Failed [%d %s]",
          status,
      ...
      response.on("end", function() {
        if (str && str.length) {
          if (str == database.checkValue) {
            console.log(
              ('Database "' + database.type + '" is up to date').green
            );
            database.skip = true;
          } else {
          ...
          console.log(
            "ERROR".red + ": Could not retrieve checksum for",
            database.type,
            "Aborting".red
          );
  ...
  function(err) {
    if (err) {
      ...
      if (args.indexOf("debug") !== -1)
        console.log(
          "Notice: temporary files are not deleted for debug purposes.".bold
            .yellow
        );
      else rimraf(tmpPath);
      process.exit(0);
