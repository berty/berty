...
class SettingsScreenBase extends PureComponent {
  ...
  onEdit = () => {
    this.setState({ 'edit': true }, () => this.props.navigation.setParams({ 'state': this.state }))
  }
  ...
  onSave = () => {
    ...
    this.setState({ 'edit': false }, () => this.props.navigation.setParams({ 'state': this.state }))
  }

  onChoosePicture = async (event) => this.setState({ ...this.state, ...(await choosePicture(event)) })
  ...
  render () {
    const { conversation, edit, t, navigation, context } = this.props
    ...
    if (otherContacts.length === 1) {
      oneToOneContact = otherContacts[0].contact
    }
          ...
          {!edit && (
            <Menu.Section>
              <Menu.Item
                ...
                onPress={() => navigation.navigate('chats/settings/notifications')
                }
              />
              <Menu.Item
                icon='clock'
          ...
          {oneToOneContact ? (
            <Menu.Section>
              <Menu.Item
                ...
                onPress={() => navigation.navigate('chats/contact/detail/list', {
            ...
            <Menu.Section title={`${members.length} members`}>
              <Menu.Item
                ...
                color={colors.blue}
                onPress={() => navigation.navigate('chats/settings/add-member', {
...
const SettingsScreen = withRelayContext(withNavigation(withNamespaces()(SettingsScreenBase)))
