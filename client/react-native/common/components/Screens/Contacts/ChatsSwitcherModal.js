...
class SwitcherListComponentBase extends React.PureComponent {
  ...
  render () {
    ...
    return <FilteredListContext.Provider value={{ ...this.state }}>
      <View style={{
        borderRadius: 10,
        margin: 10,
        marginBottom: 0,
      }}>
        <Text onChangeText={query => this.setQuery(query)} input={{
    ...
    render () {
      ...
      >
        ...
        <Flex.Rows
          ...
          style={[marginLeft]}
        >
          <Text color={index === focusedIndex ? colors.white : colors.fakeBlack} left middle>
            {title}
          </Text>
        </Flex.Rows>
      </Flex.Cols>
...
class Item extends React.PureComponent {
  render () {
    return <FilteredListContext.Consumer>{({ listProps: { focusedIndex, query } }) => {
      return <ItemBase {...this.props} focusedIndex={focusedIndex} query={query} />
