...
func MakeCertificate(cryptoImpl keypair.Interface, publicKey []byte, identityID []byte, parentCertificate *keypair.Certificate, parentEventHash []byte) (*keypair.Certificate, error) {
...
func (m *SigChain) Init(cryptoImpl keypair.Interface, identityID []byte) error {
...
func (m *SigChain) GetCertificateForIdentity(identityID []byte) (*keypair.Certificate, error) {
	for _, event := range m.Events {
		if (event.EventType == SigEvent_ADD_DEVICE || event.EventType == SigEvent_INIT_CHAIN) && bytes.Equal(event.Subject, identityID) {
...
func (m *SigChain) AddDevice(cryptoImpl keypair.Interface, selfIdentityID, identityID []byte, publicKey []byte) error {
...
func (m *SigChain) RemoveDevice(cryptoImpl keypair.Interface, selfIdentityID, identityID []byte) error {
	...
	for _, event := range m.Events {
		...
		case SigEvent_ADD_DEVICE, SigEvent_INIT_CHAIN:
			...
			if event.EventType == SigEvent_ADD_DEVICE {
				if _, ok := devices[string(certificate.Content.Issuer)]; ok == false {
					return devices, errors.New("sigchain: issuer device not found")
				}

				issuerCert = devices[string(certificate.Content.Issuer)]
			} else {
			...
			devices[string(certificate.Content.Subject)] = certificate
		...
		case SigEvent_REMOVE_DEVICE:
			...
			if _, ok := devices[string(revocation.Content.Issuer)]; ok == false {
				return devices, errors.New("sigchain: subject device not found")
			}

			if _, ok := devices[string(revocation.Content.Subject)]; ok == false {
				return devices, errors.New("sigchain: subject device not found")
			}

			issuerCert := devices[string(revocation.Content.Issuer)]

			if err := keypair.CheckSignatureUsingCert(revocation, issuerCert); err != nil {
				return devices, err
			}

			delete(devices, string(revocation.Content.Subject))
		}
