...
func TestEnclave(t *testing.T) {
	...
	Convey("Test package", t, func() {
		Convey("Keytypes satisfy keypair.Interface and return valid pubkeys", func() {
			...
			Convey("ECC Hardware keypair", nil)
			Convey("RSA Software keypair", nil)
			Convey("ECC Software keypair", nil)
			Convey("generic keypair", func() {
				kp, err := New(KeyOpts{Size: 1024})
				So(err, ShouldBeNil)
