...
func (m *Metrics) Libp2PPing(ctx context.Context, contactID string) (bool, error) {
	newCtx, cancel := context.WithTimeout(context.Background(), 2*time.Second)
	defer cancel()

	peerInfo, err := dhtcskv.ContactIDToPeerInfo(ctx, m.driver.dhtCskv, contactID)
	if err != nil {
	...
	if err := m.driver.Connect(newCtx, peerInfo); err != nil {
		return false, err
	}

	ch, err := m.driver.PingSvc.Ping(newCtx, peerID)
	if err != nil {
		return false, err
	}
