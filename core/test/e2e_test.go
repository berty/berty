...
var p2pNetworkBootstrapConfig = &dht.BootstrapConfig{
	Queries: 5,
	Period:  time.Duration(time.Second),
	Timeout: time.Duration(10 * time.Second),
}

func setupP2PNetworkDriver(bootstrap ...string) (*p2pNetwork.Driver, error) {
	return p2pNetwork.NewDriver(
		context.Background(),
		...
		p2pNetwork.WithDHTBoostrapConfig(p2pNetworkBootstrapConfig),
		p2pNetwork.WithListenAddrStrings("/ip4/127.0.0.1/tcp/0"),
		p2pNetwork.WithBootstrapSync(bootstrap...),
	)
}

func TestWithP2PNetwork(t *testing.T) {
	var (
		alice, bob, eve      *AppMock
		err                  error
		internalCtx          = context.Background()
	...
	Convey("End-to-end test (with p2p network)", t, FailureHalts, func() {
		...
		Convey("Initialize nodes", FailureHalts, func() {
			aliceNetwork, err := setupP2PNetworkDriver()
			So(err, ShouldBeNil)
			alice, err = NewAppMock(&entity.Device{Name: "Alice's iPhone"}, aliceNetwork)
			So(err, ShouldBeNil)

			bobNetwork, err := setupP2PNetworkDriver()
			So(err, ShouldBeNil)
			bob, err = NewAppMock(&entity.Device{Name: "iPhone de Bob"}, bobNetwork)
			So(err, ShouldBeNil)

			eveNetwork, err := setupP2PNetworkDriver()
			So(err, ShouldBeNil)
			eve, err = NewAppMock(&entity.Device{Name: "Eve"}, eveNetwork)
			So(err, ShouldBeNil)
		})
